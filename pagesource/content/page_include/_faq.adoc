...
== Autocompletion 'on tab' seems slow

Close all babun instances (kill all pending processes if necessary, e.g. git-credentials-.*.exe) and run `rebase.bat` which is located in babun's root folder.
